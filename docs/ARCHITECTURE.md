...
- **Onboarding is resumable.** `OnboardingRepository` keeps the current step and answers in a separate small DataStore as they change, so process death or leaving the app returns to the same step. Finishing creates the first task (with a reminder only if the person allowed reminders and chose a time) and one starter tracker per chosen focus area (study, habit, daily movement), then marks onboarding done and clears the draft. The night-stage colors live in `Stage`. Behind every step is the brand sunrise photo (`onboarding_sunrise`), framed per step by `SunriseBackdrop` and dimmed wherever text sits. The logo and wordmark are the official artwork (`kairos_emblem`, `kairos_wordmark`, cut from the logo with transparent edges), also used for the launcher icon and splash.
- **One privacy policy, two outputs.** The policy's text lives with the other strings in `tools/strings_source.py`. The app renders it natively (`PrivacyPolicyScreen`), and `tools/gen_privacy_html.py` renders `docs/privacy-policy.html` for hosting. Change the text once and regenerate both; update the date in both places (`PRIVACY_UPDATED`, `PolicyUpdated`) when the meaning changes.
